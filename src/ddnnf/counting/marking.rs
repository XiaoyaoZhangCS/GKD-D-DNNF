...
impl Ddnnf {
    ...
    pub(crate) fn calc_count_marked_node(&mut self, i: usize) {
        ...
        let temp = self.nodes[i].temp.clone();
        if temp == 0 {
            for index in 0..self.nodes[i].parents.len() {
                let parent = self.nodes[i].parents[index];
                match self.nodes[parent].ntype {
                    And {..} => {
                        self.nodes[parent].marker = false;
    ...
    pub fn mark_assumptions(&mut self, indexes: &[usize]) {
        // println!("{}",self.md.len());
