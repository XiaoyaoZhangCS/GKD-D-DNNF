...
impl Ddnnf {
    ...
    pub fn my_check(&mut self) {
        ...
        // check 有多少这样的节点：
        // 一个或节点，他的两个儿子全都是叶子节点：
        let mut or_repeat_count = 0;
        for idx in 0..self.nodes.len() {
            match self.nodes[idx].ntype.clone() {
                Or {children} => {
                    let mut flag = 0;
                    for child in children {
                        let ntype = self.nodes[child].ntype.clone();
                        match ntype {
                            And {..} => {
                                flag = 1;
                            }
        ...
        println!("两个儿子都是叶子节点的or节点的数量是:{}",or_repeat_count);
        // 一个节点，只有一个且节点作为儿子：
        let mut one_and_count = 0;
        let mut one_or_count = 0;
        for idx in 0..self.nodes.len() {
            match self.nodes[idx].ntype.clone() {
                And { children } => {
                    if children.len() == 1 {
        ...
        println!("{}个且节点只有一个儿子",one_and_count);
        println!("{}个或节点只有一个儿子",one_or_count);
        // check是不是乘法和加法交替：
        let mut count = 0;
        for idx in 0..self.nodes.len() {
            match self.nodes[idx].ntype.clone() {
        ...
        println!("{}个是交替的",count);
        //count 有多少个菱形结构：
        let mut sat : HashSet<usize> = HashSet::new();
        let mut count = 0;
        for idx in 0..self.nodes.len() {
            match self.nodes[idx].ntype.clone() {
                Or { children } => {
                    if children.len() == 2 {
                        let son1 = self.nodes[children[0]].clone();
                        let son2 = self.nodes[children[1]].clone();
                        match son1.ntype {
                            And{children} => {
                                // 把son1的儿子加入sat中：
                                sat.clear();
