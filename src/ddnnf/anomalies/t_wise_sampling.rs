...
pub fn save_solution_to_file(solution:&[i32], file_path: &str) -> io::Result<()> {
    let file_path = Path::new(file_path);
    if let Some(dir) = file_path.parent() {
        fs::create_dir_all(dir)?;
    }
    ...
    // let string_solution = solution.iter().map(|&num| num.to_string()).collect::<Vec<String>>();
    for i in solution.iter() {
        wtr.write_record(vec![i.to_string()])?;
    }
    // wtr.write_record(string_solution)?;
