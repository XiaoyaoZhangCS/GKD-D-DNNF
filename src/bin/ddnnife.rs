...
use ddnnf_lib::ddnnf::anomalies::t_wise_sampling::save_solution_to_file;
...
enum Operation {
    ...
    CountQueries {
        ...
        #[arg(short, long, value_parser = clap::value_parser!(u16).range(1..=64), default_value_t = 1, verbatim_doc_comment)]
        jobs: u16,
    },
    ...
    // #[clap(short_flag = 'k')]
    // KthSolution {
    //     /// the kth solution you want to query
    //     #[arg(verbatim_doc_comment)]
    //     kth: String,
    //     /// Default output file is '{FILE_NAME}-queries.csv'.
    //     #[arg(verbatim_doc_comment)]
    ...
    #[clap(short_flag = 'k')]
    Project {
        /// the projcet solution you want to query
        #[arg(verbatim_doc_comment)]
        queries_input_file: String,
        /// Default output file is '{FILE_NAME}-queries.csv'.
        #[arg(verbatim_doc_comment)]
    ...
    #[clap(short_flag = 'w')]
    Weighted {
        /// the weighted solution you want to query
        #[arg(verbatim_doc_comment)]
        queries_input_file: String,
        /// the weighted_file_path
        #[arg(verbatim_doc_comment)]
...
fn main() {
    ...
    if cli.pipe_ddnnf_stdin {
        ...
        let mut iter = 3;
        if(cli.m.is_some()){
            m = cli.m.clone().unwrap();
        }
        if(cli.n.is_some()){
            n = cli.n.clone().unwrap();
        }
        if(cli.iter.is_some()){
            iter = cli.iter.clone().unwrap();
        }
        ddnnf = dparser::my_build_ddnnf(ddnnf_path, cli.total_features,m,n,iter);
    }
    ...
    if cli.operation.is_some() {
        ...
        let output_file_path: String = match &operation {
            ...
            Weighted {
                custom_output_file, ..
            } => construct_ouput_path(custom_output_file, "weighted", "csv"),
            // KthSolution {
            //     kth,
            //     custom_output_file,
            // } => construct_ouput_path(custom_output_file, format!("{}th-solution", kth).as_str(), "csv"),
            Project {
                custom_output_file, ..
            } => construct_ouput_path(custom_output_file, "project", "csv"),
            Sat {
                custom_output_file, ..
        ...
        match &operation {
            ...
            Weighted {
                queries_input_file,weight_file_path, ..
            } => {
                ddnnf.init_weight(weight_file_path.clone());
                compute_queries(
                    &mut ddnnf,
                    queries_input_file,
                    &output_file_path,
                    Ddnnf::my_weighted_query,
                );
            }
...
fn compute_queries<T: std::fmt::Debug + Ord + Send + 'static>(
    ddnnf: &mut Ddnnf,
    queries_file: &String,
    output_file: &String,
